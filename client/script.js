...
function checkPiecesChosen() {
  if(currentTurn.length === 2) {
    ...
    if((currentTurn[0] === currentTurn[1]) && (!piecesFound.includes(currentTurn[0]))){
      // A successful match
...
function markPieceAsFound(value){
  if(!piecesFound.includes(value)){    // This is for NOT found
...
function unlockFoundTiles(){
  ...
  for(let count = 0; count < 2; count++){
    ...
    cells[(cellIDVals[count])].style.backgroundImage = 
      `url(${imageArray[path].img})`;
  }
}
...
function checkForEndOfGame(arr){
  ...
  if(arr.length === numOfHieros) {
    ...
    const returnObj = [{user: username.innerText,
      seconds: getSecondsFromDifference(now - start)
    }];
